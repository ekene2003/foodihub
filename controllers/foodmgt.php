...
if(isset($_POST['createFood'])){
    ...
    $description = $db->($_POST['description']);
...
if(isset($_POST['updateFood'])){
    ...
    $valid_extension = ['jpg', 'jpeg', 'png'];
    if(in_array($ext, $valid_extension)){
        $insertFood = updateFcn($table, "foodname='$foodname',price='$price',quantity='$quantity',category_id='$category_id',description='$description',vendor_id='$vendor_id',image='$image'","WHERE food_id = ?","i",$food_id);
        // $insertQuery = $db->prepare("INSERT INTO $table (foodname,price,quantity,category_id,description,vendor_id,image) VALUES (?,?,?,?,?,?,?)");
        // $insertQuery->bind_param("sssisis", $foodname,$price,$quantity,$category_id,$description,$vendor_id,$image);
        // $insertFood = $insertQuery->execute();
        if($insertFood){
            move_uploaded_file($_FILES['image']['tmp_name'], "$folder/images/$image");
            echo json_encode(['status' => true, 'title' => "style Created Successfully", 'comment' => 'We now have a new style...', 'icon' => 'success', 'btn' => 'btn btn-success']);
        }
        else{
            echo json_encode(['status' => false, 'title' => 'Unable to Create Account', 'comment' => "Darn! We are unable to create your style record.", 'icon' => 'error', 'btn' => 'btn btn-error']);
        }
    }
    else{
        echo json_encode(['status' => false, 'title' => 'Invalid image extension', 'comment' => 'Invalid Image extension. Only jpg, jpeg or png are allowed', 'icon' => 'error', 'btn' => 'btn btn-error']);
...
if(isset($_POST['updateFood'])){
    ...
    if(in_array($ext, $valid_extension)){
        ...
        // $insertFood = $insertQuery->execute();
        if($insertFood){
               isset($_FILES['image']) ? move_uploaded_file($_FILES['image']['tmp_name'], "$folder/images/$image") : "";
            echo json_encode(['status' => true, 'title' => "style updated Successfully", 'comment' => 'We now have a new style...', 'icon' => 'success', 'btn' => 'btn btn-success']);
        }
        else{
            echo json_encode(['status' => false, 'title' => 'Unable to update Account', 'comment' => "Darn! We are unable to update your style record.", 'icon' => 'error', 'btn' => 'btn btn-error']);
        }
    }
    else{
        echo json_encode(['status' => false, 'title' => 'Invalid image extension', 'comment' => 'Invalid Image extension. Only jpg, jpeg or png are allowed', 'icon' => 'error', 'btn' => 'btn btn-error']);
        ...
        $image = $row["image"];
         $deleteFood = deleteFcn($table,"WHERE food_id = ? AND vendor_id = ?","ii",$food_id,$user_id);
            if ($deleteFood) {
                ...
                echo json_encode(['status' => true, 'title' => "$foodname Deleted Successfully", 'comment' => 'you have deleted this style', 'icon' => 'success', 'btn' => 'btn btn-success']);
            }
        else{
            echo json_encode(['status' => false, 'title' => 'Unable to delete style', 'comment' => 'Sorry, we could not process your request ', 'icon' => 'error', 'btn' => 'btn btn-error']);
        }
    }
    else{
        echo json_encode(['status' => false, 'title' => 'Unable to find style', 'comment' => 'Sorry, it appears this style was not uploaded by you', 'icon' => 'error', 'btn' => 'btn btn-error']);
    }
}
?>